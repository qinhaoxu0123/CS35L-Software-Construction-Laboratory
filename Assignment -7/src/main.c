...
/*Define scene as the global variable*/
scene_t scene;
...
/*Array that hold the value that need to be printed*/
float printVals[width][height][NUM_COLORS];

/*Function to be multithreaded*/
void * mult_threads(void * NUM_thread)
{
    Vec3 camera_pos;
...
{ 
  ...
  for(int t = 0; t < nthreads; t++)
    {
      thread_id[t] = t;
      int ret = pthread_create(&threads[t], NULL, mult_threads,&thread_id[t]);
      if(ret)
        {
